...
FROM Players p
	...
	WHERE p.Rank=(SELECT MIN(Rank) FROM Players)
ORDER BY p.Rank

SELECT * 
  FROM Players pl
  JOIN PlayerStats pls
  ON pl.Id = pls.PlayerId
  JOIN Matches match
  ON pls.MatchId = match.Id
  Join Tournaments tr
  ON match.TournamentId = tr.Id
  WHERE tr.Name='Roland Garros' AND match.Round='QUARTER-FINALS'
